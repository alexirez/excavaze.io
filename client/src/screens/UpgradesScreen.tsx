...
export default function UpgradesScreen({ onBack, purchasedUpgrades = [] }: Props) {
  ...
  const canvasW = Math.max(...UPGRADE_NODES.map(n => n.x)) + 200 + CANVAS_PAD
  const canvasH = Math.max(...UPGRADE_NODES.map(n => n.y)) + 200 + CANVAS_PAD

  useEffect(() => {
    // wait for nodes to render then draw connectors
  ...
  function drawConnectors() {
    ...
    UPGRADE_NODES.forEach(node => {
      node.parents.forEach(pid => {
        ...
        const fx = fr.left - containerRect.left + scroll.scrollLeft + fr.width / 2
        const fy = fr.top - containerRect.top + scroll.scrollTop + fr.height
        const tx = tr.left - containerRect.left + scroll.scrollLeft + tr.width / 2
        const ty = tr.top - containerRect.top + scroll.scrollTop
